...
#TODO mediumint should autoincrement...
    ...
    def set_song_fingerprinted(self, sid):
        ...
        """
        with self.cursor() as cur:
            #print (self.UPDATE_SONG_FINGERPRINTED, (sid,))
    ...
    def insert(self, hash, sid, offset):
        ...
        """
        with self.cursor() as cur:
            #print (self.INSERT_FINGERPRINT, (hash, sid, offset))
    ...
    def insert_song(self, songname, file_hash):
        ...
        """
        with self.cursor() as cur:
            #print (self.INSERT_SONG, (songname, file_hash))
    ...
    def return_matches(self, hashes):
        ...
        with self.cursor() as cur:
            for split_values in grouper(values, 1000):
                split_values= list(map(lambda x: x.lower(), split_values))
                # Create our IN part of the query
                query = self.SELECT_MULTIPLE
                query = query % ', '.join(['?'] * len(split_values))
                cur.execute(query, split_values)
                values = cur.fetchall()
                for (hash, sid, offset) in values:
                    # (sid, db_offset - song_sampled_offset)
                    try:
                      #print (sid, offset - mapper[hash])
                      yield (sid, offset - mapper[hash])
                    except KeyError:
                      print "mapper does not contain key %s" % hash
                      pass
